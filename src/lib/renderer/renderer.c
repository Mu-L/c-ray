...
static void print_stats(const struct world *scene) {
	...
	uint64_t vertices = 0;
	uint64_t normals = 0;
	for (size_t i = 0; i < v_arr_len(scene->instances); ++i) {
		if (instance_type(&scene->instances[i]) == CR_I_MESH) {
			const struct mesh *mesh = &scene->meshes[scene->instances[i].object_idx];
			polys += v_arr_len(mesh->polygons);
			vertices += v_arr_len(mesh->vbuf.vertices);
			normals += v_arr_len(mesh->vbuf.normals);
		}
	}
	logr(info, "Totals: %"PRIu64"V, %"PRIu64"N, %zuI, %"PRIu64"P, %zuS, %zuM\n",
		   vertices,
		   normals,
		   v_arr_len(scene->instances),
		   polys,
		   v_arr_len(scene->spheres),
		   v_arr_len(scene->meshes));
}
...
void update_cb_info(struct renderer *r, struct tile_set *set, struct cr_renderer_cb_info *i) {
	...
	uint64_t remainingTileSamples = (v_arr_len(set->tiles) * r->prefs.sampleCount) - completed_samples;
	...
	i->completion = r->prefs.interactive ?
		...
		((double)set->finished / (double)v_arr_len(set->tiles));
...
void update_toplevel_bvh(struct world *s) {
	...
	for (size_t i = 0; i < v_arr_len(s->instances); ++i) {
		struct instance *inst = &s->instances[i];
		inst->bbuf = &s->shader_buffers[inst->bbuf_idx];
	}
...
void renderer_render(struct renderer *r) {
	...
	if (v_arr_len(r->state.clients) == 0 && r->prefs.threads < 1) {
	...
	for (size_t i = 0; i < v_arr_len(r->scene->shader_buffers); ++i) {
		if (!v_arr_len(r->scene->shader_buffers[i].bsdfs)) {
			logr(warning, "bsdf buffer %zu is empty, patching in placeholder\n", i);
			v_arr_add(r->scene->shader_buffers[i].bsdfs, build_bsdf_node((struct cr_scene *)r->scene, NULL));
		}
	}
	...
	// TODO: v_arr
	struct cr_tile *info_tiles = calloc(v_arr_len(set.tiles), sizeof(*info_tiles));
	struct cr_renderer_cb_info cb_info = {
		.tiles = info_tiles,
		.tiles_count = v_arr_len(set.tiles),
	...
	for (size_t c = 0; c < v_arr_len(r->state.clients); ++c)
		client_threads += r->state.clients[c].available_threads;
	if (v_arr_len(r->state.clients))
		logr(info, "Using %zu render worker%s totaling %zu thread%s.\n", v_arr_len(r->state.clients), PLURAL(v_arr_len(r->state.clients)), client_threads, PLURAL(client_threads));
	...
	if (r->prefs.interactive && !v_arr_len(r->state.clients)) local_render_thread = render_thread_interactive;
	
	// Create & boot workers (Nonblocking)
	// Local render threads + one thread for every client
	for (size_t t = 0; t < r->prefs.threads; ++t) {
		v_arr_add(r->state.workers, (struct worker){
			.renderer = r,
			.buf = result,
			.cam = camera,
	...
	if (threads_not_supported) {
		...
		while (r->state.s == r_rendering) {
			...
			v_timer_sleep_ms(r->state.workers[0].paused ? paused_msec : active_msec);
		}
	}
...
void renderer_destroy(struct renderer *r) {
	...
	v_arr_free(r->state.workers);
	v_arr_free(r->state.clients);
